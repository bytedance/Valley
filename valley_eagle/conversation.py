            ...
            for i, (role, message) in enumerate(messages):
                ...
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
                    if i == 0:
                        message = wrap_sys(self.system) + message if self.system is not None else message
                    if i % 2 == 0:
            ...
            for i, (role, message) in enumerate(messages):
                ...
                if message:
                    if type(message) is tuple:
                        message, _, _ = message
                    if i == 0:
                        ret = wrap_sys(self.system) + self.sep2 if self.system is not None else ""
                    if i % 2 == 0:
                        message = wrap_inst(message)
                        ret += message
                    else:
...
conv_belle = Conversation(
    system="你是字节跳动训练的大型语言视觉助手 Chinese-Valley。" "你能够理解用户提供的视觉内容或视频，并使用自然语言协助用户完成各种任务。" "请仔细按照人类的指令进行回答，并详细解释你的答案。",
    roles=("Human", "Assistant"),
    messages=[],
    offset=0,
    sep_style=SeparatorStyle.SINGLE,
