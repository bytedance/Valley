    ...
    def __init__(self, dim_in, dim_out, pixelshuffle_downsample_ratio, mlp_hidden_dim=None):
        ...
        if mlp_hidden_dim is None:
            self.mlp = nn.Sequential(
                nn.Linear(int(dim_in * (pixelshuffle_downsample_ratio**2)), dim_out), nn.GELU(), nn.Linear(dim_out, dim_out)
            )
        else:
            self.mlp = nn.Sequential(
                nn.Linear(int(dim_in * (pixelshuffle_downsample_ratio**2)), mlp_hidden_dim), nn.GELU(), nn.Linear(mlp_hidden_dim, dim_out)
            )
...
class OvisConvAdapter(nn.Module):
    ...
    def tokenize(self, logits):
        ...
        if self.tokenize_function == "softmax":
            ...
            raise ValueError(f"Invalid `max_type`, expected softmax or gumbel_argmax or st_argmax, but got {self.config.tokenize_function}")
        return tokens
...
def build_vision_projector(config, delay_load=False, **kwargs):
    ...
    if projector_type == "linear":
        ...
        return MlpPixelShuffle(
            config.mm_hidden_size, config.hidden_size, config.pixelshuffle_downsample_ratio, getattr(config, "mlp_hidden_dim", None)
        )
