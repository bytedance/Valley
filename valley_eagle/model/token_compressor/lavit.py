...
class LavitTokenCompressor(nn.Module):
    ...
    It uses Gumbel-Softmax sampling to select the tokens to keep. The number of tokens to keep in each image is UNCERTAIN.
    ...
    def __init__(self, embed_dim=2048, inner_dim=64, **kwargs):
        ...
        self.out_conv = nn.Sequential(nn.Linear(embed_dim, inner_dim), nn.GELU(), nn.Linear(inner_dim, 2), nn.LogSoftmax(dim=-1))
