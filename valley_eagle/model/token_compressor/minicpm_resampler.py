...
class MiniCPMResampler(nn.Module):
    ...
    def __init__(
        ...
        if ckpt_path is not None:
            try:
                ...
                # E722 fixed by using a specific exception class (Exception) instead of a bare 'except'
    ...
    def forward(self, x, tgt_sizes=None):
        if tgt_sizes is None:  # default mode, input square image
            assert type(x) == torch.Tensor, "only support tensor input"
...
def _mha_shape_check(
    query: Tensor, key: Tensor, value: Tensor, key_padding_mask: Optional[Tensor], attn_mask: Optional[Tensor], num_heads: int
):
    ...
    if query.dim() == 3:
        ...
        if attn_mask is not None:
            assert attn_mask.dim() in (2, 3), (
                "For batched (3-D) `query`, expected `attn_mask` to be `None`, 2-D or 3-D" f" but found {attn_mask.dim()}-D tensor instead"
            )
    elif query.dim() == 2:
        # Unbatched Inputs
        is_batched = False
